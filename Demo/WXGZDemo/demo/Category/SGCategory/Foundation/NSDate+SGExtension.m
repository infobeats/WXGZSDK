...
+ (instancetype)dy_dateFromString:(NSString *)dateString WithFormat:(NSString *)format {
    NSDateFormatter *dateFormatter=[[NSDateFormatter alloc] init];
    dateFormatter.dateFormat = format;
    return [dateFormatter dateFromString:dateString];
}

+ (NSString *)dy_currentDateStringWithFormat:(NSString *)format {
    return [[self date] dy_dateStringWithFormat:format];
}

- (NSString *)dy_dateStringWithFormat:(NSString *)format {
...
#pragma mark - 获取当前的时间
+ (NSString *)dy_currentDateString {
    return [self dy_currentDateStringWithFormat:@"yyyy-MM-dd HH:mm:ss"];
}
+ (double )dy_currentTimestamp2
{
    NSDate *date = [NSDate dateWithTimeIntervalSinceNow:0];
    /** 当前时间距1970的秒数。*1000 是精确到毫秒，不乘就是精确到秒 */
...
+ (NSString *)currentYearString {
    return [self dy_currentDateStringWithFormat:@"yyyy"];
}
+ (NSString *)currentMonthString {
    return [self dy_currentDateStringWithFormat:@"MM"];
}
+ (NSString *)currentDayString {
    return [self dy_currentDateStringWithFormat:@"dd"];
}
...
+ (NSString *)dy_dateStringWithDelta:(NSTimeInterval)delta {
    return [[NSDate dateWithTimeIntervalSinceNow:delta] dy_dateStringWithFormat: @"yyyy-MM-dd HH:mm:ss"];
}

//计算两个日期之间的天数
+ (NSInteger)dy_deltaDaysFrombeginDate:(NSString *)beginDateString endDate:(NSString *)endDateString {
    //创建日期格式化对象
    NSDate *beginDate = [self dy_dateFromString:beginDateString WithFormat:@"yyyy-MM-dd"];
    NSDate *endDate   = [self dy_dateFromString:endDateString WithFormat:@"yyyy-MM-dd"];
...
+ (NSString *)dy_dateDescriptionWithTargetDate:(NSString *)dateStr andTargetDateFormat:(NSString *)dateFormatStr {
    // 1.获取当前时间
